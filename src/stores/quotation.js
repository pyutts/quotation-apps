...
export const useQuotationStore = defineStore('quotation', () => {
  ...
  const fullContentHtml = computed(() => {
    ...
    const cName = companyName.value || 'Your Company'
    ...
    const logoHtml = logoSrc.value
      ? `<img src="${logoSrc.value}" class="max-h-20 mb-6 object-contain" style="max-height:80px;margin-bottom:24px;">`
      : ''
    ...
    return `
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-top:32px;margin-bottom:64px" class="avoid-break">
        ...
        <div style="text-align:right">
          <h1 style="font-size:48px;font-weight:900;color:#e5e7eb;text-transform:uppercase;letter-spacing:0.15em;margin-bottom:24px">Quote</h1>
          <table style="margin-left:auto;text-align:left;font-size:15px">
            <tr>
              <td style="padding-right:32px;padding-top:6px;padding-bottom:6px;color:#6b7280;font-weight:500">Quote No:</td>
              <td style="font-weight:600;color:#111827">${qNum}</td>
            </tr>
            <tr>
              <td style="padding-right:32px;padding-top:6px;padding-bottom:6px;color:#6b7280;font-weight:500">Date:</td>
      ...
      <div style="margin-bottom:56px" class="avoid-break">
        <h3 style="font-size:13px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-spacing:0.15em;margin-bottom:12px">Bill To</h3>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:15px;margin-bottom:40px" class="pdf-table">
        <thead>
          <tr>
            <th style="width:60px;text-align:center;text-transform:uppercase;letter-spacing:0.1em;font-size:12px;color:#6b7280;border-bottom:1px solid #e5e7eb;padding-bottom:12px;font-weight:600">Qty</th>
            <th style="text-align:left;text-transform:uppercase;letter-spacing:0.1em;font-size:12px;color:#6b7280;border-bottom:1px solid #e5e7eb;padding-bottom:12px;font-weight:600">Description</th>
            <th style="width:140px;text-align:right;text-transform:uppercase;letter-spacing:0.1em;font-size:12px;color:#6b7280;border-bottom:1px solid #e5e7eb;padding-bottom:12px;font-weight:600">Unit Price</th>
            <th style="width:140px;text-align:right;text-transform:uppercase;letter-spacing:0.1em;font-size:12px;color:#6b7280;border-bottom:1px solid #e5e7eb;padding-bottom:12px;font-weight:600">Amount</th>
          </tr>
        </thead>
      ...
      <div style="display:flex;justify-content:flex-end;margin-bottom:80px;margin-top:32px" class="avoid-break">
        <div style="width:320px">
          <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #f3f4f6">
            <span style="color:#6b7280;font-weight:500;font-size:15px">Subtotal</span>
            <span style="font-weight:600;color:#111827;font-size:15px">${sym}${formattedMoney(subtotal.value)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #f3f4f6">
            <span style="color:#6b7280;font-weight:500;font-size:15px">Tax (${taxRate.value || 0}%)</span>
          ...
          <div style="display:flex;justify-content:space-between;padding:20px;margin-top:16px;background:#f9fafb;border-radius:16px;border:1px solid #f3f4f6">
            <span style="font-weight:700;font-size:20px;color:#111827">Total</span>
            <span style="font-weight:700;font-size:20px;color:#7c3aed">${sym}${formattedMoney(total.value)}</span>
          </div>
        </div>
      </div>

      <!-- Footer: Terms & Signature -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:64px" class="avoid-break">
        <div>
          <h3 style="font-size:13px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:0.15em;margin-bottom:12px">Terms & Conditions</h3>
          <p style="color:#6b7280;font-size:13px;line-height:1.6;padding-right:32px">
            Please make payment within 14 days of receiving this quote. All prices are subject to taxes as stated. Quote valid for 30 days. Let us know if you have any questions.
          </p>
        </div>
        <div style="text-align:right;display:flex;flex-direction:column;align-items:flex-end;padding-top:16px">
          ...
          <p style="font-weight:700;color:#111827;font-size:15px">Authorized Signature</p>
